...
const OPENAI_API_KEY = " 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";
const API_URL = "https://api.openai.com/v1/completions";

const windowWidth = Dimensions.get("window").width;

export default function StudyTipsScreen({ navigation, route }) {
  const [inputText, setInputText] = useState(
    "I have an important exam coming up next week. Can you give me some tips on effective time management for studying?"
  );
  ...
  useEffect(() => {
    setBotRole(
      "Welcome to the world of Nexus, your study buddy extraordinaire! I'm an intelligent chat bot named Nexus, designed by Chatto to help you succeed in your studies. Whether you're preparing for an exam, tackling a challenging subject, or looking for effective study strategies, I'm here to provide you with valuable tips and advice. From time management techniques to note-taking strategies, I've got you covered. Just let me know what you need assistance with, and together we'll conquer the realm of learning!"
    );
  }, []);
  ...
  const [conversation, setConversation] = useState(route.params?.history || []);

  const saveConversation = async (updateConversation) => {
    try {
      const newHistory = [...conversationHistories, updateConversation];
          ...
          if (typingIndex < botResponse.length) {
            setBotTypingText(
              (prevText) =>
                prevText +
                (typingIndex > 0
                  ? botResponse[typingIndex]
                  : botResponse[typingIndex].trim())
            );
            typingIndex++;
          } else {
            ...
            setConversation((prevConversation) => {
              ...
              let updatedConversation = prevConversation.map((item) => {
                if (item.type === "user" && item.content === inputText) {
                  foundMatch = true;
                  return { type: "user", content: inputText };
                } else if (item.type === "bot" && item.content.trim() === botResponse.trim()) {
                  foundMatch = true;
                  return { type: "bot", content: botResponse.trim() };
                } else {
                  return item;
                }
              });
              ...
              if (!foundMatch) {
                updatedConversation.push({ type: "user", content: inputText }); // Add new user item
                updatedConversation.push({ type: "bot", content: botResponse.trim() }); // Add new bot item
              }
              ...
              const saveConversationHistory = async (updatedConversation) => {
                try {
                  ...
                  // Check if the updated conversation already exists in histories
                  const existingIndex = histories.findIndex((history) =>
                      JSON.stringify(history) === JSON.stringify(prevConversation)
                  );

                  if (existingIndex !== -1) {
                    // If the updated conversation exists, update the conversation array
                    histories[existingIndex] = updatedConversation;
                  } else {
                    // If the updated conversation does not exist, add it to histories
